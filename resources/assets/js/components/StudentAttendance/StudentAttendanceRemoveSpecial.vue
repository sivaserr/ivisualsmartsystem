...
<form @submit.prevent="removeHourAttendance()">
     ...
     <div class="select-class-only">
                ...
                <input  style="min-width: 151px;" required="true" type="date" id="date" v-model="date" name="date" max="3000-12-31" min="1000-01-01" class="form-control">
  ...
  <select v-model="hour" class="form-control">
      <option v-for="hour_option in hour_options">{{hour_option.value}}</option>
  </select>
</div>
            </div>
                        </div>
...
let loader = this.$loading.show({
 ...
 fetch('/api/student/removeattendance', {
                    method: "post",
