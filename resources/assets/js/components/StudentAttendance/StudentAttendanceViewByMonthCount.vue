...
<div class="ip_addres"><span>KSRCE/QM/7.5.3/31/{{department}}/{{PrintAcademicYear()}}/</span></div> 
            ...
            <table class="table table-bordered">
                <tbody>
                    ...
                    <tr v-for="(Student,sl_no) in filteredStudents" v-bind:key="Student.id">
                        ...
                        <td>{{attendanceHours(Student).overall}} / {{attendanceHours(Student).st_overall}} =
                             <span v-if="attendanceHours(Student).st_overall > 0" style="font-weight:bolds;">
                                    {{(Math.round(attendanceHours(Student).overall / attendanceHours(Student).st_overall * 100)).toFixed(1)}}%
                                </span>
                        </td>
                        <td>
...
const d = new Printd()
d.print(document.getElementById('printMonth'), [cssText] );
