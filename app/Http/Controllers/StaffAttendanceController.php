...
class StaffAttendanceController extends Controller
{
    ...
    public function getStafflimits($id){
        $aca = Academic::find(1);
        $od = StaffAttendanceRecord::where([
            ['staff_id', '=', $id],
            ['status', '=', 'od'],
            ['year_start', '=', $aca->year_start]
        ])->get()->count();
        $cl = StaffAttendanceRecord::where([
            ['staff_id', '=', $id],
            ['status', '=', 'cl'],
            ['year_start', '=', $aca->year_start]
        ])->get()->count();
        $cpl = StaffAttendanceRecord::where([
            ['staff_id', '=', $id],
            ['status', '=', 'cpl'],
            ['year_start', '=', $aca->year_start]
        ])->get()->count();
        $sod = StaffAttendanceRecord::where([
            ['staff_id', '=', $id],
            ['status', '=', 'sod'],
            ['year_start', '=', $aca->year_start]
        ])->get()->count();
        
        $data = array('od_count'=>$od, 'cl_count'=>$cl, 'cpl_count'=>$cpl, 'sod_count'=>$sod);
